...
Private Function statN(ByVal N As Long) As Object
    ...
    For Each key In freq.Keys
        Dim k As Long: k = CLng(key)
        Dim v As Long: v = freq(key)
        sum = sum + k * v
...
Public Sub 演示_数据分析()
    ...
    For cnt = 0 To maxN
        ...
        For N = 1 To maxN
            Dim freq As Object: Set freq = stats(N)("freq")
            Dim fVal As Long: fVal = dictGet(freq, cnt)
            If fVal > 0 Then
                Dim pct As Double: pct = fVal / stats(N)("total") * 100
    ...
    For N = 1 To maxN
        Set s = stats(N)
        Dim fullN As Long: fullN = dictGet(s("freq"), N)
        Dim ratio As Double
    ...
    For N = 1 To maxN
        Set s = stats(N)
        Dim z As Long: z = dictGet(s("freq"), 0)
        Dim fullN2 As Long: fullN2 = dictGet(s("freq"), N)
        ws.Cells(r, 1).Value = N
    ...
    For N = 1 To maxN
        Set s = stats(N)
        Dim f1 As Long: f1 = dictGet(s("freq"), 1)
        ws.Cells(r, 1).Value = N
    ...
    For N = 1 To maxN
        Set s = stats(N)
        Dim eMean As Double: eMean = s("mean")
        Dim eDivN As Double
