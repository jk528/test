...
' v11.6 修复：仅"横向分开"需要互换 Q2/Q3 公式，横向合并同竖向公式
' 现有功能（不变）：
...
Sub 四象限_执行(是否正向 As Boolean, 是否横向 As Boolean)
    ...
    Dim 互换 As Boolean  ' 横向分开模式标志（仅横向分开时互换 Q2/Q3 公式）
    ' =========================
    ...
    For i = 1 To 总行数
        For j = 1 To 总列数
            If 互换 Then
                Q2(j, i) = Q1(j, 总行数 - i + 1)              ' 右上=左右镜像=组合反转
                Q3(j, i) = Q1(总列数 - j + 1, i)              ' 左下=上下镜像=元素反转
            Else
                Q2(j, i) = Q1(总列数 - j + 1, i)              ' 右上=左右镜像=元素反转
                Q3(j, i) = Q1(j, 总行数 - i + 1)              ' 左下=上下镜像=行反转
            End If
            Q4(j, i) = Q1(总列数 - j + 1, 总行数 - i + 1)      ' 右下=中心镜像=双反转
        Next j
    Next i
