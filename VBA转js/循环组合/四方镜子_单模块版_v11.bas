'============================================================
' 四方镜子 - 单模块版 v11（四象限横竖双版）
' 基于 v10 扩展：完全保留现有功能不变，新增"四象限对称·横"
' 现有功能（不变）：
'   - 四方循环 scct（笛卡尔积，4个按钮）
...
'   - 四象限对称·竖（按钮8）
' 新增功能：
'   - 按钮9 "四象限对称·横" → 横向排列的四象限对称
'     布局：
'       左上：正横（左快右慢，原始数据）   右上：反横（右快左慢，原始数据）
'       左下：正横（左快右慢，反转数据）   右下：反横（右快左慢，反转数据）
'     每个组合横向排列（一行），四象限上下堆叠
...
Sub 四象限_执行(是否竖向 As Boolean)
    ...
    Dim 片段() As String
    ' 竖向分开用
    Dim 每象限列数 As Long, 右起始列 As Long
    Dim 下起始行 As Long
    ' 横向合并用
    ...
    ' 映射关系（四象限对称是内在固定的，不受数据反转开关影响）：
    '   左上 Q1 = 反向（反竖/反横）—— 基准
    '   右上 Q2 = 正向+反转（正竖/正横+数据反转）—— 左右镜像
    '   左下 Q3 = 正向（正竖/正横）—— 上下镜像
    '   右下 Q4 = 反向+反转（反竖/反横+数据反转）—— 中心镜像
    ' 对称关系：
    '   上下对称：反向 ↔ 正向（方向反转）
    '   左右对称：反向 ↔ 正向+反转（方向+数据双反转）
    '   中心对称：反向 ↔ 反向+反转（数据反转）
    '
    ' 数据反转开关的作用：把源数据整体上下颠倒后再生成四象限
    ' （相当于四个象限各自的内容都上下翻转，但位置不变）
    If 数据是否反转 Then
        生成scct结果 Q1, 反转数据, 每列行数, 总列数, 总行数, False  ' 反向（左上基准，源数据已反转）
        生成scct结果 Q2, 源数据, 每列行数, 总列数, 总行数, True    ' 正向+反转（右上，左右镜像）
        生成scct结果 Q3, 反转数据, 每列行数, 总列数, 总行数, True   ' 正向（左下，上下镜像）
        生成scct结果 Q4, 源数据, 每列行数, 总列数, 总行数, False   ' 反向+反转（右下，中心镜像）
    Else
        生成scct结果 Q1, 源数据, 每列行数, 总列数, 总行数, False  ' 反向（左上基准）
        生成scct结果 Q2, 反转数据, 每列行数, 总列数, 总行数, True  ' 正向+反转（右上，左右镜像）
        生成scct结果 Q3, 源数据, 每列行数, 总列数, 总行数, True   ' 正向（左下，上下镜像）
        生成scct结果 Q4, 反转数据, 每列行数, 总列数, 总行数, False ' 反向+反转（右下，中心镜像）
    End If
    ...
    If 是否竖向 Then
        ...
        If 是否合并 Then
            ' 合并模式：每象限1列字符串
            ' 列布局：A=左列两象限 B=空 C=右列两象限
            ' 行布局：第1行=上标签 2~M+1=上半 M+2=空 M+3=下标签 M+4~2M+3=下半
            ...
            ' 标签
            新表.Cells(1, 1).Value = "【左上 " & 反名 & "】"
            新表.Cells(1, 3).Value = "【右上 " & 正名 & "+反转】"
            新表.Cells(总行数 + 3, 1).Value = "【左下 " & 正名 & "】"
        ...
        Else
            ...
            ' 标签
            新表.Cells(1, 1).Value = "【左上 " & 反名 & "】"
            新表.Cells(1, 右起始列).Value = "【右上 " & 正名 & "+反转】"
            新表.Cells(下起始行 - 1, 1).Value = "【左下 " & 正名 & "】"
            新表.Cells(下起始行 - 1, 右起始列).Value = "【右下 " & 反名 & "+反转】"
        End If
    Else
        ' ============== 横向版：组合横向排列 ==============
        If 是否合并 Then
            ' 合并模式：每象限1行字符串（M个组合横排）
            ' 行布局：1=上标签 2=上半 3=空行 4=下标签 5=下半
            ' 列布局：B列起 左半M个=左上 Q1，M+3列起 右半M个=右上 Q2
            ReDim 上左横(1 To 总行数)
            ReDim 上右横(1 To 总行数)
            ReDim 下左横(1 To 总行数)
            ReDim 下右横(1 To 总行数)
            ...
            新表.Cells(2, 右起始列横).Resize(1, 总行数) = 上右横
    ...
    MsgBox "四象限对称·" & 横竖名 & " 生成完成：" & 总行数 & " 组 × 4 象限。", vbInformation, "四象限对称"
