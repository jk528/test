...
Sub 四象限_执行(是否竖向 As Boolean)
    ...
    ' Q1=左上 正向  Q2=右上 反向  Q3=左下 正向+反转  Q4=右下 反向+反转
    Dim Q1() As Variant, Q2() As Variant, Q3() As Variant, Q4() As Variant
    生成scct结果 Q1, 源数据, 每列行数, 总列数, 总行数, True   ' 正向
    生成scct结果 Q2, 源数据, 每列行数, 总列数, 总行数, False  ' 反向
    生成scct结果 Q3, 反转数据, 每列行数, 总列数, 总行数, True  ' 正向+反转
    生成scct结果 Q4, 反转数据, 每列行数, 总列数, 总行数, False ' 反向+反转
    ...
    Else
        ...
        If 是否合并 Then
            ...
            ' 标签
            新表.Cells(1, 2).Value = "【左上 " & 正名 & "】"
            新表.Cells(1, 右起始列横).Value = "【右上 " & 反名 & "】"
            新表.Cells(4, 2).Value = "【左下 " & 正名 & "+反转】"
