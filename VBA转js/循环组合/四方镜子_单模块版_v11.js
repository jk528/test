...
function 四象限_执行(是否正向, 是否横向, 连接符, 是否合并, 数据是否反转) {
    ...
    try {
        ...
        // Q2/Q3/Q4：视觉对称 = 字符串反转
        //   字符串反转在数据层面 = 组合反转(R) + 元素反转(E) = 双反转(RE)
        //   变换类型: 1=R(组合反转), 2=E(元素反转), 3=RE(双反转)
        //   竖向:    Q2=E(2),  Q3=R(1),  Q4=RE(3)
        //   横向分开: Q2=R(1),  Q3=E(2),  Q4=RE(3)
        //   横向合并: Q2=RE(3), Q3=E(2),  Q4=R(1)
        var Q2 = [], Q3 = [], Q4 = [];
        var 变换表 = (是否横向 && 是否合并) ? [3, 2, 1]      // 横向合并: RE, E, R
                   : (是否横向 && !是否合并) ? [1, 2, 3]    // 横向分开: R, E, RE
                   : [2, 1, 3];                             // 竖向: E, R, RE
        function 取值(变换, c, r) {
            var jc = (变换 === 2 || 变换 === 3) ? 总列数 - 1 - c : c;  // E: 反转列(元素)
            var rc = (变换 === 1 || 变换 === 3) ? 总行数 - 1 - r : r;  // R: 反转行(组合)
            return Q1[jc][rc];
        }
        for (var c = 0; c < 总列数; c++) {
            Q2[c] = []; Q3[c] = []; Q4[c] = [];
