/*============================================================
 * 四方镜子 - JS宏 v11.6（HTML窗体版 + InputBox回退）
 * 对应 VBA v11.6 修复版：
 *   - 7 个按钮 + 3 个复选框（合并 / 数据反转 / 四象限对称）
 *   - 四象限对称：真正镜像（行反转+元素反转），开关绑定按钮1~4
 *   - v11.6 修复：横向布局下"合并/分开"镜像公式不同——仅横向分开互换 Q2/Q3
 *   - 备选算法验证（7种：A~G）
 * 使用方法：运行 四方镜子_主入口()
 * 内置测试：运行 测试生成全部四象限()
 *============================================================*/
...
function 四象限_执行(是否正向, 是否横向, 连接符, 是否合并, 数据是否反转) {
    ...
    try {
        ...
        // Q2/Q3/Q4：按布局选择镜像公式
        // 视觉上的"左右/上下"是相对最终输出布局而言的：
        //   竖向（每行一组合，元素从左到右）：左右镜像=元素反转；上下镜像=行反转
        //   横向分开（K行×M列，元素从上到下）：左右镜像=组合反转；上下镜像=元素反转
        //   横向合并（1行字符串，元素从左到右）：左右镜像=元素反转；上下镜像=行反转（同竖向）
        //   中心镜像=两者都反转（所有布局公式相同）
        //   即：只有"横向分开"才互换 Q2/Q3 公式，其他情况都用竖向公式
        var Q2 = [], Q3 = [], Q4 = [];
        var 互换 = (是否横向 && !是否合并);  // 仅横向分开需要互换
        for (var c = 0; c < 总列数; c++) {
            Q2[c] = []; Q3[c] = []; Q4[c] = [];
            for (var r = 0; r < 总行数; r++) {
                if (互换) {
                    Q2[c][r] = Q1[c][总行数 - 1 - r];              // 右上=左右镜像=组合反转
                    Q3[c][r] = Q1[总列数 - 1 - c][r];              // 左下=上下镜像=元素反转
                } else {
                    Q2[c][r] = Q1[总列数 - 1 - c][r];              // 右上=左右镜像=元素反转
                    Q3[c][r] = Q1[c][总行数 - 1 - r];              // 左下=上下镜像=行反转
                }
                Q4[c][r] = Q1[总列数 - 1 - c][总行数 - 1 - r];      // 右下=中心镜像=双反转
            }
        }
...
// ============================================================
//  测试：用 2x3x4 数据生成全部 4 种基准 × 2 种合并 = 8 个四象限
//  运行：测试生成全部四象限()
// ============================================================
...
function 测试生成全部四象限() {
    ...
    Application.ScreenUpdating = false;
    try {
        // 1. 新建测试数据表（A列2行, B列3行, C列4行 = 24组合）
        var 测试表 = Worksheets.Add({ Before: Application.Sheets.Item(1) });
        测试表.Name = "测试数据_2x3x4";
        var 数据 = [
            ["a1", "b1", "c1"],
            ["a2", "b2", "c2"],
        ...
        测试表.Range("A1").Resize(4, 3).Value2 = 数据;
        Application.ActiveSheet = 测试表;

        // 2. 4 种基准 × 2 种合并 = 8 次
        var 基准列表 = [
            { 名: "反竖", 正向: false, 横向: false },
            { 名: "正横", 正向: true,  横向: true  },
            { 名: "正竖", 正向: true,  横向: false },
            { 名: "反横", 正向: false, 横向: true  }
        ];
        var 合并列表 = [true, false];

        var 计数 = 0;
        for (var b = 0; b < 基准列表.length; b++) {
            for (var m = 0; m < 合并列表.length; m++) {
                var 基准 = 基准列表[b];
                var 合并 = 合并列表[m];
                测试表.Activate();  // 每次调用前重新激活测试数据表
                四象限_执行(基准.正向, 基准.横向, "-", 合并, false);
                计数++;
            }
        }

        Application.Alert("测试完成：已生成 " + 计数 + " 个四象限结果表（4基准×2合并，2x3x4=24组合/象限）。请检查各表对称性。", "四象限测试");
    } finally {
        Application.ScreenUpdating = 原刷新;
    }
}
