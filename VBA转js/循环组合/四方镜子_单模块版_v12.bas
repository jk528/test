'============================================================
' 四方镜子 - 单模块版 v12（非模态窗体，关闭后才退出清理）
' 基于 v11.6 改造：窗体改为 vbModeless 非模态，按钮点击不关闭窗体
' 只有用户点 X 关闭窗体时，DoEvents 循环退出，才清理组件
'
' 核心变化：
'   - 非模态窗体：打开时可同时操作 Excel
'   - 按钮执行后保持窗体打开，可连续操作
'   - 关闭窗体（点X）后自动清理动态创建的组件
' 现有功能（不变）：
...
Private Sub 注入事件代码(窗体组件 As Object)
    ...
    ' ---- QueryClose：点X关闭时卸载窗体，触发 DoEvents 循环退出 ----
