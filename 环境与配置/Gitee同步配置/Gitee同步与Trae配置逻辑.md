...
│      ├─ .git/                    ← Git 本地仓库         │
│      ├─ .gitignore                ← 忽略规则            │
│      ├─ Gitee同步配置/             ← Gitee 配置文档     │
│      └─ 业务文件 (自动化与工具/VBA转js/、语料与文本/小说/ 等)                   │
└─────────────────────────────────────────────────────────┘
              │
              │ git push / pull (SSH)
              ▼
...
2. **项目内备用**：`Gitee同步配置/.git-ssh-config`
   ```
   Host gitee.com
       HostName gitee.com
       User git
...
3. **临时使用**（如需在 Trae 内测试）：
   ```powershell
   $env:GIT_SSH_COMMAND = "ssh -F C:/Users/Administrator/Documents/这是什么/JK-temp/环境与配置/Gitee同步配置/.git-ssh-config"
   git fetch origin
   ```
...
**原因**：使用了废弃的 29418 端口
**解决**：config 中改 `Port 22`；URL 改 scp 格式 `git@gitee.com:jk528528/test.git`
...
| 备用 SSH config | `Gitee同步配置/.git-ssh-config` | 项目内备用配置 |
| 项目规则 | `.trae/rules/project_rules.md` | Trae AI 行为约束 |
