...
def split_by_chapter(src, out_dir="", prefix="", serial_width=3, encoding=None,
    ...
    # 4.5 预计算章节字数（统计全部字符，含标题和换行，用于文件名显示）
    char_counts = []
    for idx in range(n_total):
        s_line = starts[idx]
        e_line = starts[idx + 1] - 1 if idx + 1 < n_total else len(lines) - 1
        if e_line < s_line:
            e_line = s_line
        char_counts.append(len("\n".join(lines[s_line:e_line + 1])))
    max_chars = max(char_counts) if char_counts else 0
    char_width = max(1, len(str(max_chars)))
    print("章节字数: 最大%d字  字数位数: %d" % (max_chars, char_width))
    ...
    for idx in range(n_total):
        ...
        if prefix:
            fname = "%s_%s_%s_%s.txt" % (prefix, serial, char_cnt, safe_title)
        else:
            fname = "%s_%s_%s.txt" % (serial, char_cnt, safe_title)
