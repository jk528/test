// PC_002_排列组合.js - WPS环境JavaScript宏
...
function RunSingleTest(ws, rowIdx, funcName, baseArr, n) {
    try {
        ...
        if (Array.isArray(Arr)) {
            if (Arr[0] === '' || Arr[0] === undefined) {
                for (var ti = 1; ti < Arr.length; ti++) {
                    if (Arr[ti] !== undefined && Arr[ti] !== null) {
                        m = Arr.length - 1;
                        break;
                    }
                }
...
function 自测_组合排列_自动验证() {
    try {
        ...
        ws.Columns.AutoFit();
        Application.MsgBox("自测完成，请查看工作表【组合排列_自测报告】。", 64, "提示");
    } catch (e) {
        Application.MsgBox("自测发生错误: " + (e.message || String(e)), 16, "错误");
    }
}
...
function 组合排列_范围选取_示例() {
    try {
        var selectedRange = Application.InputBox("请选择数据区域（支持多列）:", "组合排列输入", "", 100, 100, "", 0, 8);
        if (selectedRange === false) return;
        if (!selectedRange) {
            Application.MsgBox("选择区域无有效数据。", 48, "警告");
            return;
        }
        ...
        if (isNaN(OP) || OP < 1 || OP > 4) {
            Application.MsgBox("无效选择（请输入 1~4）。", 48, "警告");
            return;
        }
        var m = Arr.length - 1;
        if (Arr[0] === '' || Arr[0] === undefined) {
        ...
        var comboRangeInput = Application.InputBox("组合长度：输入 all（全部：1..n）或 单值k 或 区间 k1-k2", "组合长度", "all", 100, 100, "", 0, 2);
        if (comboRangeInput === false || comboRangeInput === "") return;
        var rangeResult = ParseComboRange(comboRangeInput, m);
        if (!rangeResult) {
            Application.MsgBox("组合长度输入非法，请输入 all、单值k或区间k1-k2（范围：1..n）。", 48, "警告");
            return;
        }
        ...
        ws.Columns.AutoFit();
        Application.MsgBox("已生成结果到工作表【" + ws.Name + "】。" + "\n" +
                           "范围：" + kStart + " - " + kEnd + "；类型：" + funcName, 64, "提示");
    } catch (e) {
        Application.MsgBox("发生错误: " + (e.message || String(e)), 16, "错误");
    }
}

function Initialize() {
    console.log("PC_002_排列组合 代码初始化完成");
    console.log("可用函数:");
